...
The object returned from `nnt()` is a list including the same components that are returned from `RANN::nn()`, `RANN::nn2()` and `nabor::knn()`, that is, matrices containing the nearest neighbour distances (`nn.dists`) and the corresponding indices in `data` (`nn.idx`).  The $i$th row relates to the $i$th query point, the $i$th row of `query`. 
...
The argument `fn` can be used to choose the function that finds nearest neighbour distances.  Possibilities are `nabor::knn()` (L2 metric) and `RANN.L1:nn2()` (L1 metric).  Any function can be used provided that it has syntax consistent with a call `fn(data = data, query = query, k = k, ...)`.  The following code produce the same output as `fn = RANN::nn2()`.
